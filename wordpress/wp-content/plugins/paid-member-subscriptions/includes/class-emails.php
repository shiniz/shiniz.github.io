...
Class PMS_Emails {
    ...
    static function send_emails( $subscription_id = 0, $subscription_data = array(), $old_subscription_data = array() ) {
        ...
        // Send emails only if the status changes
        ...
        // Check that the status is a supported email action
        ...
        if( empty( $settings['emails']['admin_emails_on'] ) )
            return;
    ...
    static function pms_mail( $send_to = '', $action = '', $user_id = 0, $subscription_plan_id = 0, $start_date = '', $expiration_date = '' ) {
        ...
        if( $send_to == 'admin' ) {
            ...
            $admin_emails = ( ! empty( $settings['emails']['admin_emails'] ) ? $settings['emails']['admin_emails'] : '' );
        ...
        // Set email subject
        if( ! empty( $settings['emails'][$action . '_sub_subject' . $settings_sufix] ) )
            $email_subject = $settings['emails'][$action . '_sub_subject' . $settings_sufix];
        else
            $email_subject = $email_default_subjects[$action];

        // Set email message
        if( ! empty( $settings['emails'][$action . '_sub' . $settings_sufix] ) )
            $email_content = $settings['emails'][$action . '_sub' . $settings_sufix];
        else
