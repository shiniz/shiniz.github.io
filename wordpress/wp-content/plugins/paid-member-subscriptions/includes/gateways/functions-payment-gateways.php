...
function pms_get_output_payment_gateways( $pms_settings = array() ) {
    ...
    $active_gateways = ( ! empty( $pms_settings['payments']['active_pay_gates'] ) && is_array( $pms_settings['payments']['active_pay_gates'] ) ? $pms_settings['payments']['active_pay_gates'] : array() );
    ...
    // If there's only one payment gateway saved
        ...
        $output .= apply_filters( 'pms_output_payment_gateway_input_hidden', '<input type="hidden" name="pay_gate" value="' . esc_attr( $paygate_key ) . '" ' . $gateway_supports_arr[$paygate_key] . ' />', $active_gateways[0] );
        ...
        $default_gateway  = ( !empty( $pms_settings['payments']['default_payment_gateway'] ) ? ( in_array( $pms_settings['payments']['default_payment_gateway'], $active_gateways ) ? $pms_settings['payments']['default_payment_gateway'] : $active_gateways[0] ) : 'paypal_standard' );
            ...
            $output .= '<div id="pms-paygates-inner">';
                ...
                if( !empty( $active_gateways ) ) {
                    foreach( $active_gateways as $paygate_key ) {
                        ...
                        $output .= '<label>';
                            $output .= apply_filters( 'pms_output_payment_gateway_input_radio', '<input type="radio" name="pay_gate" value="' . esc_attr( $paygate_key ) . '" ' . checked( $default_gateway, $paygate_key, false ) . $gateway_supports_arr[$paygate_key] . ' />', $paygate_key );
