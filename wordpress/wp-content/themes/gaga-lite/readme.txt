License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Install Steps:
--------------

1. Activate the theme
2. Go to the Customizer Page
3. Setup customizer options


------------------------------------------

JS Files
    
    BXSlider by Steven Wanderski: 
    License: WTFPL and MIT license: WTFPL and MIT license
    https://github.com/stevenwanderski/bxslider-4
	
	Classy Loader(Circular-percentage-loader)
    License:MIT www.class.pm/LICENSE-MIT
    https://vox.space/files/jquery/classyloader/
    2005 - 2015 © vox.SPACE
    By Marius Stanciu
    
    JQuery One Page Nav Plugin
    License: MIT and GPL
    http://github.com/davist11/jQuery-One-Page-Nav
    By:Trevor Davis
    
    Localscroll
    License: MIT
    https://github.com/flesler/jquery.localScroll
    Copyright (c) 2007-2015 Ariel Flesler
    
    Waypoints
    License: MIT License
    https://github.com/imakewebthings/waypoints
    Copyright © 2011-2015 Caleb Troughton
    
    Parallax JS 
    License: MIT and GPL licenses
    https://github.com/IanLunn/jQuery-Parallax/
    By: Ian Lunn
    
    WOW JS
    License: MIT Licenses
    https://github.com/matthieua/WOW
    
    Flexisel
    License: MIT License
    https://github.com/9bitStudios/flexisel
    
    Jquery.scrollTo
    license: MIT License
    https://github.com/flesler/jquery.scrollTo/
    By Ariel Flesler
    
    Jquery.nav
    License:the MIT and GPL
    http://github.com/davist11/jQuery-One-Page-Nav
    Copyright (c) 2010 Trevor Davis (http://trevordavis.net)
    
    MixItUp
    License: Commercial use requires a commercial license.
    https://mixitup.kunkalabs.com
    Copyright Copyright 2015 KunkaLabs Limited
    
    MaxImage
    Dual licensed under the MIT and GPL licenses.
    http://www.aaronvanderzwan.com/maximage/2.0/
    Copyright (c) 2007-2012 Aaron Vanderzwan
-----------------------------------------------------------
CSS File
    BXSlider CSS
    by Steven Wanderski: 
    License: WTFPL and MIT license: WTFPL and MIT license
    https://github.com/stevenwanderski/bxslider-4
        
    Flexisel CSS
    License: MIT License
    https://github.com/9bitStudios/flexisel
    
    WOW CSS
    License: MIT Licenses
    https://github.com/matthieua/WOW
-------------------------------------------
Fonts
    Font Awesome 4.4.0 by @davegandy - http://fontawesome.io - @fontawesome
    License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
        
    Lato, by Warsaw-based designer Lukasz Dziedzic: SIL Open Font License, 1.1  
    https://www.google.com/fonts/specimen/Lato
    
    Oswald by Alternate Gothic' sans serif typefaces: SIL Open Font License, 1.1
    https://www.google.com/fonts/specimen/Oswald
    
    Arvo by Anton Koovit: SIL Open Font License, 1.1
    https://www.google.com/fonts/specimen/Arvo    
   
---------------------------------------------
Images
    All the images used are is designed by AccessPress team.
        
---------------------------------------------
Documentation
    Setup Home Page
        -Go to Page->Add New  give the name of page and select home page template and save.
        -Go to Setting->Reading-> A static page (select below) and select page what we vreate for home.
    
    Add Parallax Section
        -Go to Appearance -> Customizer -> Home Page Setting. Enable the section and configure the necessary setting.

==== Update Logs ===

Version 1.3.2
* Minor Bug Fixes

Version 1.3.1
* Fixed Woocommerce compatibility issues with the latest version of woocommerce

Version 1.3.0
* Added Support for Custom Editor Style
* Fixed Woocommerce Comapatibility issues
* Added Welcome Page with demo import functionality

Version 1.2.9
* Section Background Issue Fixed

Version 1.2.8
* Woocommerce Page issue solve 

Version 1.2.7
* Update Sanitizeation Function
* Update Prefixing 

Version 1.2.6
* Shop And Portfolio Backgeound Issue Fixed

Version 1.2.5
* Header top menu issue slove

Version 1.2.4
* Add More theme information at customizer

Version 1.2.3
* Page breadcrumb issue Solve

Version 1.2.2
* Backend customizer option issue solve

Version 1.2.1
* Update Header Banner title issue

Version 1.1.9
* Update service section css
* Update Woocommerce css
 
Version 1.1.8
* Update page title bg issue
* Update Dynamic css 

Version 1.1.6
*Update dynamic css issue

Version 1.1.5
*FullScreen Slider
*Woocommerce Compatible
*Team section design issue solved
*Update Font Awesome

Version 1.1.3
* Update Customizer options
* Add option to choose BG Image for body
* Update css design
* Update Dynamic css

Version 1.1.1
* RTL Issue Solve
* Dynamic Issue Solve

Version 1.0.9
* Fix Responsive issue
* Home page slider js bug fixed
* Header menu design issue fixed
* Logo link issue solve

Version 1.0.8
* Minor Design and Programming Bug fixex
* Responsive bug fixed


Version 1.0
* Submitted theme for review in http://wordpress.org